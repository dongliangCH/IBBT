function [min_cost_idx, goalP0, min_cost, Traj] = ConnectGoalprune(Vertices, goal_cord, BeliefNodes, param, world, r)
        ...
        if size(BeliefNodes,1) >= near_idx(i)
         ...
         if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
             for k = 1:size(BeliefNodes{near_idx(i)},2)               
                 ...
                 [endP0, endPtilde, CovCost, CollisionProb, K] = propagate( BeliefNodes{near_idx(i)}{k}{1}, BeliefNodes{near_idx(i)}{k}{2}, N, param, meanTraj, world ); 
