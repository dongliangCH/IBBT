...
rng;
randM = rand(1,6000); randTheta = rand(1,3000); rand_idx = 1; randvel_idx = 1;
...
for l = 2:samples
    ...
    while sample_succ == 0
        ...
        if dim == 2
            new_point(dim + 1) = 2 * pi * randTheta(randvel_idx);     % Theta [0, 2*pi]
        ...
        if collision_point(new_point, world) == 0
            ...
            if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                for k = 1:size(BeliefNodes{idx},2)
                    ...
                    [endP0, endPtilde, CovCost, CollisionProb, K] = propagate( BeliefNodes{idx}{k}{1}, BeliefNodes{idx}{k}{2}, param, meanTraj, world ); 
