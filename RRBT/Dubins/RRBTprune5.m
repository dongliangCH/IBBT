function [Time, Cost] = RRBTprune5

clear
addpath('CS', 'DubinsCurve');
deter_vertix = load('deterministicVertices');
rng(0);
dim = 2;   
...
% start_cord = [9,6,0];
start_cord = [3,8,pi/6];
P0 = diag([0.10, 0.08, 0.08]);
% Initial estimation error covariance
PtildePrior0 = 0.6 * P0.* diag([1 1 1]);
% Initial estimated state covariance
PhatPrior0 = P0 - PtildePrior0;

% goal_cord = [17,18,0];
goal_cord = [18,18,pi/2];
...
%                 randomPoint(1, j) = world.origincorner(j) + (world.endcorner(j) - world.origincorner(j)) * rand;
                randomPoint(1, j) = deter_vertix.Vertices(l,j);
            end
        ...
        if dim == 2
%             new_point(dim + 1) = 2*pi*rand;
            new_point(dim + 1) = deter_vertix.Vertices(l, 3);
        end
        ...
        if collision_point(new_point, world) == 0
            ...
            if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                for k = 1:size(BeliefNodes{idx},2)
                    ...
                    [~, ~, ~, CollisionProb, ~] = propagate( BeliefNodes{idx}{k}{1}, BeliefNodes{idx}{k}{2}, param, meanTraj, world ); 
                            ...
                            for i = 1 : size_near                                              
                                if near_idx(i) ~= idx                                    
                                    ...
                                    if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                                        ...
                                        Edges_data{near_idx(i), new_verti_idx} = {meanTraj, MCost};
                                    end
                                    ...
                                    if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                                        ...
                                        Edges_data{new_verti_idx, near_idx(i)} = {meanTraj, MCost};
                                    end
...
function [Belief_queue, BeliefNodes] = SearchGraph(Edges, Edges_data, Belief_queue, BeliefNodes, param, world)
    ...
    while queue_size > 0
        ...
        for j = 1:size(Edges{vertix_idx},2)
            ...
            [meanTraj, MCost] = Edges_data{vertix_idx, Edges{vertix_idx}(j)}{1:end};
            [endP0, endPtilde, CovCost, CollisionProb, ~] = propagate( pop_node{1}, pop_node{2}, param, meanTraj, world );                         
            if CollisionProb <= param.chanceConstraint
