...
% Create random world
world = createKnownWorld([20, 20], [0, 0],dim);  
...
node_init(1) = {P0}; node_init(2) = {PtildePrior0};  
node_init(3) = {0}; node_init(4) = {[]}; node_init(5) = {[1;1]}; node_init(6) = {[]}; % cost, child, coordinate in BeliefNodes, parent_node
...
while queue_size > 0
    ...
    for j = 1:size(Edges{vertix_idx},2)
        ...
        CovCost = 0;
        if CollisionProb <= param.chanceConstraint+0.03
