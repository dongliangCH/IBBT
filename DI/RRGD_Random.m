function [Vertices, Edges, Edges_data, Belief_queue_next]= RRGD_Random(Vertices, Edges, Edges_data, Belief_queue_next_old, BeliefNodes, ...
...
for k = 1:samples
    ...
    while sample_succ == 0
        ...
        if dim == 2
            new_point(dim + 1 : 2 * dim) = 0.5 * (-1 + (1 - (-1))) * randMvel(randvel_idx,:);     % vmin = -1£¬ vmax = 1
        ...
        if collision_point(new_point, world) == 0
            ...
            if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                sample_succ = 1;
                        ...
                        Edges_data{idx, new_verti_idx} = {meanTraj, V, MCost, N, Xbar};
                        ...
                        if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                            ...
                            Edges_data{new_verti_idx,idx} = {meanTraj, V, MCost, N, Xbar};
                            plot(meanTraj(1,:), meanTraj(2,:), 'color', 'g', 'LineWidth', 1);
                        end
                            ...
                            for i = 1 : size_near                                              
                                if near_idx(i) ~= idx
                                    ...
                                    if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                                        ...
                                        Edges_data{near_idx(i), new_verti_idx} = {meanTraj, V, MCost, N, Xbar};
                                        plot(meanTraj(1,:), meanTraj(2,:), 'color', 'g', 'LineWidth', 1);
                                    ...
                                    if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                                        ...
                                        Edges_data{new_verti_idx, near_idx(i)} = {meanTraj, V, MCost, N, Xbar};
                                        plot(meanTraj(1,:), meanTraj(2,:), 'color', 'g', 'LineWidth', 1);
                                    end
                                end
                            end             
