...
while z ~= goal_idx
    ...
    for x = NN{z}(W(NN{z}))
        ...
        Tfs = zeros(1, m);
        for i=1:m
            [seg_cost(i), Tfs(i)] = segment_cost(State(:,Y_near(i)), State(:,x), param);
        end
        [c_min, y_idx] = min(C(Y_near) + seg_cost);
        y_min = Y_near(y_idx);
        collision_flag = collisionKnowTf(State(:,y_min), State(:,x), world, dim, Tfs(y_idx), param);
        if ~collision_flag
            A(x) = y_min;
            C(x) = c_min;
            if plot_flag
                plot(State(1,[x, y_min]), State(2,[x, y_min]), 'ok-'); hold on
            end
            H_new(end+1) = x;
            W(x) = false;
        end
