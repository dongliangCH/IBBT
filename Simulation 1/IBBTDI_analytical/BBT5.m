...
goal_cord = [4,7,0,0];                       % goal_cord = [17,18,0,0];

world = createKnownWorld(dim);               % Create random world
plotCovariance(start_cord, P0);

velRand = rand(300, 2)-0.5;
param.dt = 0.1;
param.velavg = 1;
param.chanceConstraint = 0.1;

r = 2.2;                                     % Neighbor distance
segmentLength = 2.1;                         % Maximum steplength
...
Vertices = [ start_cord; goal_cord; deter_Vertices(3:samples, :)];                       % start_cord already in deter_vertices
