...
clear
addpath('CS');
deter_vertix = load('deterministicVertices.mat');
rng(0);
dim = 2;   

% Initial state distribution (mean/covariance)
start_cord = [1,3,0,0];
P0 = diag([0.08, 0.06, 0.01, 0.01]);
...
for l = 2:samples
    ...
    while sample_succ == 0
        ...
        min_dist = 0;
        while min_dist < 1.5
%             randomPoint = [4.4 7.4].* rand(1,2) + [0.3 0.3];
            randomPoint = deter_vertix.Vertices(l, 1:2);
        ...
        if dim == 2
%             new_point(dim + 1 : 2 * dim) = rand(1,2) - 0.5;
        ...
        if collision_point(new_point, world) == 0
            ...
            if ~MeanCollisionCheck(meanTraj(1:2, :), world, dim)
                for k = 1:size(BeliefNodes{idx},2)
                    ...
                    [~, ~, ~, CollisionProb, ~] = propagate( BeliefNodes{idx}{k}{1}, BeliefNodes{idx}{k}{2}, N, param, meanTraj, world ); 
                    if CollisionProb < param.chanceConstraint - 0.05
    ...
    if Connected_flag && mod(Iter, 5)==0
        Time = [Time toc];
        BeliefTrees = [BeliefTrees {BeliefNodes}];
        TreesVertices = [TreesVertices, {Vertices}];
    end    
    % check if connected to goal
    if ~Connected_flag && (norm(new_point-goal_cord)<0.1 || Connected(Vertices, goal_cord, BeliefNodes, param, world, r))       
        Connected_flag = 1;
        Time = [Time toc];
        BeliefTrees = [BeliefTrees {BeliefNodes}];
        TreesVertices = [TreesVertices, {Vertices}];
            ...
            if CollisionProb <= param.chanceConstraint
                ...
                if Edges{vertix_idx}(j) > size(BeliefNodes,1)  % Edges{vertix_idx}(j) is the newly added vertix, there is no node at this vertix and the new node is directly added
                    ...
                    for m = 1:size(BeliefNodes{Edges{vertix_idx}(j)},2)
                        ...
                        if cost_to_come + 0.01 > BeliefNodes{Edges{vertix_idx}(j)}{m}{3} && ...
