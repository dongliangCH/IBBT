...
PtildePrior0 = 0.6 * P0.* diag([1 1 1]);     % Initial estimation error covariance
PhatPrior0 = P0 - PtildePrior0;              % Initial estimated state covariance

goal_cord = [18,18,pi/2];                    % goal_cord = [17,18,0];
fig = figure(1);
...
for l = 1:samples
    ...
    while min_dist < 1
        ...
        [min_dist, ~] = min(sum((pos - randomPoint).^2,2)); % find the vertix that is closest to randomPoint
    end   
    pos = [pos; randomPoint];
end
vel = [start_cord(3); goal_cord(3); rand(samples, 1)*2*pi];
...
if param.saveAnim
    ...
    for idx = 1:size(im,2)
        ...
        if idx == 1
            imwrite(A,map,filename,"gif","LoopCount",1,"DelayTime",1);
        else
